...
Extensions can be installed into the usual path at `/opt/hivemq/extensions/`. Note that this image uses https://github.com/GoogleContainerTools/distroless[distroless] and therefore does not have a shell to use in your Docker build.

To utilize `RUN` statements to configure your image beforehand, you can use https://docs.docker.com/develop/develop-images/multistage-build/[multi stage builds].

To install an extension, you can simply use the `COPY` directive in your `Dockerfile`. For example, to install the (already extracted) https://www.hivemq.com/extension/file-rbac-extension/[file RBAC extension].
