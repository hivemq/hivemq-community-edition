...
IMPORTANT: Until HiveMQ Community Edition 2020.3 is released embedded mode is only available from link:https://jitpack.io/#hivemq/hivemq-community-edition[jitpack.io].
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        final EmbeddedExtension embeddedExtension = EmbeddedExtensionBuilder.builder()
        ...
        final EmbeddedHiveMQBuilder builder = EmbeddedHiveMQBuilder.builder()
                .withConfigurationFolder(Path.of("/path/to/embedded-config-folder"))
                .withDataFolder(Path.of("/path/to/embedded-data-folder"))
                .withExtensionsFolder(Path.of("/path/to/embedded-extensions-folder"))
