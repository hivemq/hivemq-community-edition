...
public class IsolatedPluginClassloader extends URLClassLoader {
    ...
    private static final ImmutableSet<Class> classesWithStaticContext = new ImmutableSet.Builder<Class>().add(
            Services.class,
            Builders.class
    ).build();

    private static final ImmutableSet<String> classNamesWithStaticContext = new ImmutableSet.Builder<String>().add(
    ...
    public void loadClassesWithStaticContext() {
        ...
        for (final Class staticClass : classesWithStaticContext) {
            ...
            try (final InputStream resourceAsStream = staticClass.getResourceAsStream(staticClass.getSimpleName() + ".class")) {
    ...
    @Override
    @NotNull
    protected synchronized Class<?> loadClass(@NotNull final String name, final boolean resolve) throws ClassNotFoundException {
