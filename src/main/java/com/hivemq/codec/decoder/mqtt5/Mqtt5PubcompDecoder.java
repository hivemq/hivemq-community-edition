...
public class Mqtt5PubcompDecoder extends AbstractMqttDecoder<PUBCOMP> {
    ...
    @VisibleForTesting
    @Inject
    public Mqtt5PubcompDecoder(final Mqtt5ServerDisconnector disconnector, final FullConfigurationService fullConfigurationService) {
    ...
    public PUBCOMP decode(final Channel channel, final ByteBuf buf, final byte header) {
        ...
        if (!validateHeader(header)) {
            disconnector.disconnect(channel,
                    ...
                    "Invalid PUBCOMP fixed header", Mqtt5DisconnectReasonCode.PROTOCOL_ERROR,
                    ReasonStrings.DISCONNECT_PROTOCOL_ERROR_PUBCOMP_HEADER);
            return null;
        }
