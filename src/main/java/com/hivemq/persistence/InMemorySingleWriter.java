 ...
 *         RxJava Schedulers.trampoline() The advantage is that there are less thread switches (resulting in context
 *         switches). The requirement is that no submitted task is blocking (that is only true for in-memory
 *         persistences)
 */
