...
public interface ClientQueuePersistence {
    ...
    /**
     * Add a publish to the queue. The publish will be queued without a packet ID
     *
     * @param queueId    of the queue
     * @param shared     is true if the client is actually a shared subscription
     * @param publish    to add
     * @param retained   true if this message was sent in response to a subscribe. It is not necessarily the same as the
    ...
    /**
     * Add a list of publishes to the queue. The publishes will be queued without packet IDs
     *
     * @param queueId    of the queue
     * @param shared     is true if the client is actually a shared subscription
     * @param publishes  to add
     * @param retained   true if this message was sent in response to a subscribe. It is not necessarily the same as the
     *                   retain flag of the publishes.
     * @param queueLimit of the client session or the default configuration.
     */
    @NotNull
    ListenableFuture<Void> add(
    ...
    /**
     * Read publishes that are not yet in-flight. Sets the given packet ID's for the returned publishes if qos > 0. The
     * amount of packet ID's is also the limit for the read batch.
     *
     * @param  queueId   of the queue
    ...
    /**
     * Replace a publish with a pubrel of the same packet ID. If no publish is found for this ID, it is stored
     * regardless.
     *
     * @param client   of the queue
    ...
    /**
     * Read publishes that are not yet in-flight up to the provided limit. The messages are marked as in-flight.
     *
    ...
    /**
     * Remove the in-flight marker of a PUBLISH which has the same unique ID as the one that is provided. This way the
     * PUBLISH will be return by future calls of
     * {@link ClientQueuePersistence#readNew(String, boolean, ImmutableIntArray, long)}.
     * <p>
     * This method is only used for shared subscription queues.
     *
     * @param sharedSubscription for which the messages should be removed
