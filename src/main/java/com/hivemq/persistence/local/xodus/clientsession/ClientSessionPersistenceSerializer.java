...
public class ClientSessionPersistenceSerializer {
    ...
    public byte[] serializeValue(final ClientSession clientSession, final long timestamp) {
        ...
        if (willPublish != null) {
            ...
            // payload ID, delay, ttl, payload format, retained, topic length, topic, hivemq id length, hivemq id , user
            // properties
            willLength += 8 + 8 + 8 + 1 + 1 + 4 + topicLength + 4 + hivemqIdLength
    ...
    public ClientSession deserializeValue(final byte[] bytes) {
        ...
        if (willQos != NO_WILL_MARKER) {
            ...
            cursor += 1;
            willBuilder.withPayloadFormatIndicator(
                    payloadFormatCode != -1 ? Mqtt5PayloadFormatIndicator.fromCode(payloadFormatCode) : null);
