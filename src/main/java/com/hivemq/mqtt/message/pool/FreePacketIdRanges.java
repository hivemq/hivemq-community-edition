...
/**
 * The purpose of this class is to reduce packet IDs allocation time for each independent client and to reduce the
 * memory footprint of keeping track of allocated IDs.
 * <p>
 * This is achieved by keeping a list of {@link Range} objects each of which represents a contiguous interval of integer
 * ids that are NOT currently assigned to any object using this instance of {@link FreePacketIdRanges}. The lower end of
 * the {@link Range} instance is included whereas the upper end is excluded from the interval. That is, the
 * {@link Range} instance with start 42 and end 42 contains only one ID - 42. The {@link Range} instance with start 10
 * and end 12 contains only two IDs: 10 and 11.
 * <p>
 * Initially, there is only one contiguous {@link Range} of IDs. The IDs are assigned starting from the lowest one in
 * that interval. Upon assignment, the interval's lower end is incremented (interval size reduces from below). When the
 * ID is returned, it either joins one of the existing {@link Range} intervals in the list (if it is adjacent) or it
 * forms a new {@link Range} that is added to the list. <a href=
 * "https://github.com/hivemq/hivemq-mqtt-client/blob/master/src/main/java/com/hivemq/client/internal/util/Ranges.java">The
...
public class FreePacketIdRanges {
    ...
    public synchronized void takeSpecificId(final int id) throws MessageIdUnavailableException {
        ...
        while (current != null) {
            if (id < current.start) {
                throw new MessageIdUnavailableException(id); // since the ranges are traversed in increasing order of
                                                             // IDs, the given id will not be found
            }
