...
/**
 * The Subscribe Message handler which is responsible for handling the subscriptions of MQTT clients
 *
...
public class SubscribeHandler extends SimpleChannelInboundHandler<SUBSCRIBE> {
    ...
    protected void channelRead0(@NotNull final ChannelHandlerContext ctx, @NotNull final SUBSCRIBE msg) throws Exception {
        processSubscribe(ctx, msg, new Mqtt5SubAckReasonCode[msg.getTopics().size()], new String[msg.getTopics().size()], false);
    }
    ...
    private void persistSubscriptionForClient(final @NotNull ChannelHandlerContext ctx,
        ...
        if (futureCount == 0) {
            ...
            ctx.writeAndFlush(new SUBACK(msg.getPacketIdentifier(), ImmutableList.copyOf(answerCodes), reasonString));
            return;
        }
