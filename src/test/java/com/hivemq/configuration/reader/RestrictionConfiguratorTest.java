...
public class RestrictionConfiguratorTest extends AbstractConfigurationTest {
    ...
    public void test_restrictions_xml() throws Exception {
        final String contents = "<hivemq>" + "<restrictions>" + "<max-connections>500</max-connections>" +
                "<max-client-id-length>400</max-client-id-length>" + "<max-topic-length>400</max-topic-length>" +
                "<no-connect-idle-timeout>300</no-connect-idle-timeout>" +
                "<incoming-bandwidth-throttling>200</incoming-bandwidth-throttling>" + "</restrictions>" + "</hivemq>";
    ...
    public void test_restriction_negative_values() throws Exception {
        final String contents = "<hivemq>" + "<restrictions>" + "<max-connections>-100</max-connections>" +
                "<max-client-id-length>-100</max-client-id-length>" + "<max-topic-length>-100</max-topic-length>" +
                "<no-connect-idle-timeout>-100</no-connect-idle-timeout>" +
                "<incoming-bandwidth-throttling>-100</incoming-bandwidth-throttling>" + "</restrictions>" + "</hivemq>";
        Files.write(contents.getBytes(UTF_8), xmlFile);
        reader.applyConfig();
    ...
    public void test_tooHigh() throws IOException {
        final String contents = "<hivemq>" + "<restrictions>" + "<max-connections>500</max-connections>" +
                "<max-client-id-length>123456</max-client-id-length>" + "<max-topic-length>123456</max-topic-length>" +
                "<no-connect-idle-timeout>300</no-connect-idle-timeout>" +
                "<incoming-bandwidth-throttling>200</incoming-bandwidth-throttling>" + "</restrictions>" + "</hivemq>";
        Files.write(contents.getBytes(UTF_8), xmlFile);
        reader.applyConfig();
