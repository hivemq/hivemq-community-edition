...
public class ConnectHandlerTest {
    ...
    public void setUp() throws Exception {
        ...
        when(
                clientSessionPersistence
                        .clientConnected(anyString(), anyBoolean(), anyLong(), any(MqttWillPublish.class), anyLong()))
                .thenReturn(Futures.immediateFuture(null));
    ...
    public void test_connect_with_session_expiry_interval_zero() {
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withSessionExpiryInterval(0).withClientIdentifier("1")
                .withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
    ...
    public void test_connect_with_keep_alive_zero() {
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withKeepAlive(0).withClientIdentifier("1")
                .withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
        assertTrue(channel.isOpen());
        channel.writeInbound(connect1);
        assertTrue(channel.isOpen());
    ...
    public void test_connect_with_keep_alive_zero_not_allowed() {
        ...
        assertTrue(channel.isOpen());
        final Integer keepAlive = ClientConnection.of(channel).getConnectKeepAlive();
        final KeepAliveDisconnectHandler keepAliveDisconnectHandler = (KeepAliveDisconnectHandler) channel.pipeline()
                .get(ChannelHandlerNames.MQTT_KEEPALIVE_IDLE_HANDLER);
        assertEquals(
    ...
    public void test_connect_with_keep_alive_higher_than_server() {
        ...
        configurationService.mqttConfiguration().setKeepAliveAllowZero(false);
        createHandler();
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withKeepAlive(1000).withClientIdentifier("1")
                .withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
    ...
    public void test_connect_with_keep_alive_ok() {
        ...
        configurationService.mqttConfiguration().setKeepAliveAllowZero(false);
        createHandler();
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withKeepAlive(360).withClientIdentifier("1")
                .withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
        final AtomicLong keepAliveFromCONNACK = new AtomicLong();
        final CountDownLatch connackLatch = new CountDownLatch(1);
        channel.pipeline().addFirst(new ChannelOutboundHandlerAdapter() {
    ...
    public void test_connect_with_max_packet_size() {
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withMaximumPacketSize(300).withClientIdentifier("1")
                .withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
        assertTrue(channel.isOpen());
        channel.writeInbound(connect1);
        assertTrue(channel.isOpen());
    ...
    public void test_connect_with_session_expiry_interval_max() {
        ...
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withSessionExpiryInterval(SESSION_EXPIRY_MAX)
                .withClientIdentifier("1").withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).build();
        assertTrue(channel.isOpen());
        channel.writeInbound(connect1);
        assertTrue(channel.isOpen());
        final Long expiry = ClientConnection.of(channel).getClientSessionExpiryInterval();
    ...
    public void test_connect_with_auth_user_props() throws InterruptedException {
        ...
        final CONNECT connect1 = new CONNECT.Mqtt5Builder().withClientIdentifier("ownId")
                .withUserProperties(Mqtt5UserProperties.of(MqttUserProperty.of("connect", "value"))).build();
    ...
    public void test_connect_handler_removed_from_pipeline() {
        ...
        final CONNECT connect = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("clientId").withCleanStart(true).build();
    ...
    public void test_client_takeover_mqtt3() throws Exception {
        ...
        final EmbeddedChannel oldChannel = new EmbeddedChannel(testDisconnectHandler,
                new TestDisconnectEventHandler(disconnectEventLatch));
        ...
        final CONNECT connect1 = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("clientId").build();
        channel.writeInbound(connect1);
    ...
    public void test_too_long_clientid() throws Exception {
        ...
        final CONNECT connect = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("123456").build();
    ...
    public void test_will_topic_dollar() throws Exception {
        ...
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt3Builder().withPayload(new byte[100])
                .withQos(QoS.EXACTLY_ONCE).withHivemqId("hmqid").withTopic("top/#").build();
        final CONNECT connect = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("123456").withWillPublish(willPublish).build();
        final CountDownLatch eventLatch = new CountDownLatch(1);
        channel.pipeline().addLast(new TestDisconnectEventHandler(eventLatch));
        channel.closeFuture().addListener((ChannelFutureListener) future -> latch.countDown());
        channel.writeInbound(connect);
    ...
    public void test_will_topic_max_length_exceeded() throws Exception {
        ...
        final CountDownLatch latch = new CountDownLatch(1);
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt3Builder().withPayload(new byte[100])
                .withQos(QoS.EXACTLY_ONCE).withTopic("12345678890").build();
        final CONNECT connect = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("123456").withWillPublish(willPublish).build();
        final CountDownLatch eventLatch = new CountDownLatch(1);
        channel.pipeline().addLast(new TestDisconnectEventHandler(eventLatch));
    ...
    public void test_will_topic_max_length_exceeded_mqtt5() throws Exception {
        ...
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt5Builder().withPayload(new byte[100])
                .withQos(QoS.EXACTLY_ONCE).withTopic("12345678890").build();
        final CONNECT connect = new CONNECT.Mqtt5Builder().withClientIdentifier("123456").withWillPublish(willPublish)
                .build();
        final CountDownLatch eventLatch = new CountDownLatch(1);
        channel.pipeline().addLast(new TestDisconnectEventHandler(eventLatch));
        channel.closeFuture().addListener((ChannelFutureListener) future -> latch.countDown());
    ...
    public void test_will_exceed_max_qos_mqtt5() throws Exception {
        ...
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt5Builder().withPayload("message".getBytes())
                .withQos(QoS.EXACTLY_ONCE).withUserProperties(Mqtt5UserProperties.NO_USER_PROPERTIES).withTopic("topic")
                .build();
        final CONNECT connect = new CONNECT.Mqtt5Builder().withClientIdentifier("123456").withWillPublish(willPublish)
                .build();
        final CountDownLatch eventLatch = new CountDownLatch(1);
    ...
    public void test_will_retain_not_supported_mqtt3() throws InterruptedException {
        ...
        createHandler();
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt3Builder().withPayload(new byte[100])
                .withQos(QoS.EXACTLY_ONCE).withHivemqId("hmqid").withTopic("top").withRetain(true).build();
        final CONNECT connect = new CONNECT.Mqtt3Builder().withProtocolVersion(ProtocolVersion.MQTTv3_1_1)
                .withClientIdentifier("123456").withWillPublish(willPublish).build();
        final CountDownLatch eventLatch = new CountDownLatch(1);
        channel.pipeline().addLast(new TestDisconnectEventHandler(eventLatch));
    ...
    public void test_auth_in_progress_message_handler_is_removed() {
        ...
        final CONNECT connect = new CONNECT.Mqtt5Builder().withClientIdentifier("client").withAuthMethod("someMethod")
                .build();
    ...
    public void test_will_authorization_success() {
        ...
        final CONNECT connect = new CONNECT.Mqtt5Builder().withClientIdentifier("client").withWillPublish(willPublish)
                .build();
        defaultPermissions.add(
                new TopicPermissionBuilderImpl(new TestConfigurationBootstrap().getFullConfigurationService())
                        .topicFilter("topic").type(TopicPermission.PermissionType.ALLOW).build());
        channel.writeInbound(connect);
        final CONNACK connack = channel.readOutbound();
        assertNotNull(connack);
        assertEquals(Mqtt5ConnAckReasonCode.SUCCESS, connack.getReasonCode());
    ...
    public void test_will_authorizer_next_perms_avail_deny() {
        ...
        defaultPermissions.add(
                new TopicPermissionBuilderImpl(new TestConfigurationBootstrap().getFullConfigurationService())
                        .topicFilter("topic").type(TopicPermission.PermissionType.DENY).build());
        final PublishAuthorizerResult result = new PublishAuthorizerResult(null, null, true);
        channel.pipeline().fireUserEventTriggered(new AuthorizeWillResultEvent(connect, result));
        channel.runPendingTasks();
        final CONNACK connack = channel.readOutbound();
    ...
    public void test_start_connection_persistent() throws Exception {
        final CONNECT connect = new CONNECT.Mqtt3Builder().withClientIdentifier("client")
                .withProtocolVersion(ProtocolVersion.MQTTv3_1_1).withCleanStart(false)
                .withSessionExpiryInterval(SESSION_EXPIRY_MAX).build();
    ...
    public void test_update_persistence_data_fails() throws Exception {
        final CONNECT connect = new CONNECT.Mqtt3Builder().withClientIdentifier("client")
                .withProtocolVersion(ProtocolVersion.MQTTv3_1_1).withCleanStart(false).build();
        clientConnectionContext.setClientId("client");
    ...
    private void createHandler() {
        ...
        final Provider<PublishFlowHandler> publishFlowHandlerProvider = () -> new PublishFlowHandler(
                mock(PublishPollService.class), mock(IncomingMessageFlowPersistence.class),
                mock(OrderedTopicService.class), mock(IncomingPublishHandler.class),
                mock(DropOutgoingPublishesHandler.class));
        final Provider<FlowControlHandler> flowControlHandlerProvider = () -> new FlowControlHandler(
                configurationService.mqttConfiguration(), serverDisconnector);
        handler = new ConnectHandler(clientSessionPersistence, connectionPersistence, configurationService,
                publishFlowHandlerProvider, flowControlHandlerProvider, mqttConnacker, new TopicAliasLimiterImpl(),
                mock(PublishPollService.class), mock(SharedSubscriptionService.class), internalAuthServiceImpl,
                authorizers, pluginAuthorizerService, serverDisconnector, mock(KeepAliveDisconnectService.class));
        ...
        channel.pipeline().addBefore(
                ...
                new DummyHandler());
        doAnswer(invocation -> {
            ctx.channel().attr(ClientConnectionContext.CHANNEL_ATTRIBUTE_NAME).get()
