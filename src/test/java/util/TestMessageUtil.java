...
public class TestMessageUtil {
    ...
    public static final Mqtt5UserProperties TEST_USER_PROPERTIES = Mqtt5UserProperties
            .of(new MqttUserProperty("user1", "property1"), new MqttUserProperty("user2", "property2"));
    public static final List<String> topics = ImmutableList.of("topic1", "topic2", "topic3");
    public static PUBLISH createMqtt3Publish() {
        return new PUBLISHFactory.Mqtt3Builder().withQoS(QoS.AT_LEAST_ONCE).withOnwardQos(QoS.AT_LEAST_ONCE)
                .withTopic("topic").withPayload("payload".getBytes()).withPacketIdentifier(1).withHivemqId("hivemqId")
                .build();
    }
    ...
    public static PUBLISH createMqtt3Publish(
        ...
        return new PUBLISHFactory.Mqtt3Builder().withQoS(qoS).withOnwardQos(qoS).withTopic(topic).withPayload(payload)
                .withRetain(retain).withHivemqId(hivemqId).withPacketIdentifier(1).build();
    }

    public static PUBLISH createMqtt3Publish(final String topic, final byte[] payload, final QoS qoS) {
        return new PUBLISHFactory.Mqtt3Builder().withQoS(qoS).withOnwardQos(qoS).withTopic(topic).withPayload(payload)
                .withHivemqId("hivemqId").build();
    }
    ...
    public static PUBLISH createMqtt3Publish(
        ...
        return new PUBLISHFactory.Mqtt3Builder().fromPublish(publish).withHivemqId(hivemqId).withTimestamp(timestamp)
                .withPublishId(publishId).build();
    }

    public static PUBLISH createMqtt3Publish(final String hivemqId, final long publishid) {
        return new PUBLISHFactory.Mqtt3Builder().withQoS(QoS.AT_LEAST_ONCE).withOnwardQos(QoS.AT_LEAST_ONCE)
                .withTopic("topic").withPacketIdentifier(1).withPublishId(publishid).withHivemqId(hivemqId)
                .withPayload(new byte[]{}).build();
    }

    public static PUBLISH createMqtt3Publish(final long timestamp) {
        return new PUBLISHFactory.Mqtt3Builder().withQoS(QoS.AT_LEAST_ONCE).withOnwardQos(QoS.AT_LEAST_ONCE)
                .withTopic("topic").withTimestamp(timestamp).withHivemqId("hivemqId").withPayload(new byte[]{}).build();
    }

    public static PUBLISH createMqtt3Publish(final QoS qoS) {
        return new PUBLISHFactory.Mqtt3Builder().withQoS(qoS).withOnwardQos(qoS).withTopic("topic")
                .withPayload("payload".getBytes()).withHivemqId("hivemqId").withPacketIdentifier(1).withPublishId(1L)
                .build();
    }
    ...
    public static PUBLISH createMqtt5Publish(final String topic, final QoS qos) {
        return new PUBLISHFactory.Mqtt5Builder().withQoS(qos).withOnwardQos(qos).withTopic(topic)
                .withPayload("payload".getBytes()).withPacketIdentifier(1).withHivemqId("hivemqId").build();
    }

    public static PUBLISHFactory.Mqtt5Builder getDefaultPublishBuilder() {
        return new PUBLISHFactory.Mqtt5Builder().withQoS(QoS.AT_LEAST_ONCE).withOnwardQos(QoS.AT_LEAST_ONCE)
                .withTopic("topic").withPayload("payload".getBytes()).withPacketIdentifier(1).withPublishId(1L)
                .withHivemqId("hivemqId");
    }

    public static PUBLISH createMqtt5Publish(final int packetId) {
        return new PUBLISHFactory.Mqtt5Builder().withQoS(QoS.AT_LEAST_ONCE).withOnwardQos(QoS.AT_LEAST_ONCE)
                .withTopic("topic").withPayload("payload".getBytes()).withPacketIdentifier(packetId)
                .withHivemqId("hivemqId").build();
    }
    ...
    public static PUBLISH createMqtt5Publish(
        ...
        return new PUBLISHFactory.Mqtt5Builder().withHivemqId(hivemqId).withTopic(topic).withPayload(payload)
                .withQoS(qos).withOnwardQos(qos).withRetain(isRetain).withMessageExpiryInterval(messageExpiryInterval)
                ...
                .withUserProperties(userProperties).withPacketIdentifier(packetIdentifier).withDuplicateDelivery(isDup)
                .withNewTopicAlias(isNewTopicAlias).withSubscriptionIdentifiers(subscriptionIdentifierArray).build();
    }
    ...
    public static CONNECT createFullMqtt5Connect() {
        return new CONNECT.Mqtt5Builder().withUserProperties(TEST_USER_PROPERTIES).withClientIdentifier("clientid")
                .withKeepAlive(60).withCleanStart(true).withSessionExpiryInterval(360)
                .withResponseInformationRequested(true).withProblemInformationRequested(true).withReceiveMaximum(100)
                .withTopicAliasMaximum(10).withMaximumPacketSize(200).withUsername("username")
                .withPassword("password".getBytes()).withAuthMethod("auth method").withAuthData("auth data".getBytes())
                .withWillPublish(
                        new MqttWillPublish.Mqtt5Builder().withHivemqId("hivemqId1").withTopic("topic")
                                .withPayload("payload".getBytes()).withQos(QoS.EXACTLY_ONCE).withRetain(true)
                                .withMessageExpiryInterval(360)
                                .withPayloadFormatIndicator(Mqtt5PayloadFormatIndicator.UTF_8)
                                .withContentType("content type").withResponseTopic("response topic")
                                .withCorrelationData("correlation data".getBytes())
                                .withUserProperties(TEST_USER_PROPERTIES).withDelayInterval(60).build())
                .build();
    }

    public static CONNACK createFullMqtt5Connack() {
        return CONNACK.builder().withReasonCode(Mqtt5ConnAckReasonCode.SUCCESS).withReasonString("success")
                .withUserProperties(TEST_USER_PROPERTIES).withSessionPresent(true).withSessionExpiryInterval(720)
                .withServerKeepAlive(120).withAssignedClientIdentifier("assigned").withAuthMethod("auth method")
                .withAuthData("auth data".getBytes()).withReceiveMaximum(100).withTopicAliasMaximum(5)
                .withMaximumPacketSize(100).withMaximumQoS(QoS.AT_LEAST_ONCE).withRetainAvailable(true)
                .withWildcardSubscriptionAvailable(true).withSubscriptionIdentifierAvailable(true)
                .withSharedSubscriptionAvailable(true).withResponseInformation("response").withServerReference("server")
                .build();
    }
    ...
    public static SUBSCRIBE createFullMqtt5Subscribe() {
        ...
        topicBuilder.add(
                new Topic(topics.get(2), QoS.EXACTLY_ONCE, true, false,
    ...
    public static DISCONNECT createFullMqtt5Disconnect() {
        return new DISCONNECT(Mqtt5DisconnectReasonCode.NORMAL_DISCONNECTION, "reason", TEST_USER_PROPERTIES,
                "server reference", 360);
    }

    public static AUTH createFullMqtt5Auth() {
        return new AUTH("auth method", "auth data".getBytes(), Mqtt5AuthReasonCode.SUCCESS, TEST_USER_PROPERTIES,
                "reason");
    }
    ...
    public static CONNECT createMqtt5ConnectWithWill() {
        final MqttWillPublish willPublish = new MqttWillPublish.Mqtt5Builder().withTopic("topic")
                .withQos(QoS.EXACTLY_ONCE).withPayload(new byte[]{1, 2, 3}).build();
        return new CONNECT.Mqtt5Builder().withWillPublish(willPublish).withClientIdentifier("client")
                .withCleanStart(false).withSessionExpiryInterval(100).build();
    }
